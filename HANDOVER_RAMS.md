...
**Version:** `package.json` `version` field — plain hand-bumped semver, currently `2.3.5`. No longer auto-derived from git commit count (that mechanism was removed 2026-08-20 — it silently produced wrong numbers under Vercel's shallow git clone; see §Current State). Bumped on **every** merged change, including docs-only ones, so the in-app footer version reliably reflects what's actually deployed.
